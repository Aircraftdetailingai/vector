...
export async function notifyQuoteSent({ detailerId, quote }) {
  const aircraft = quote.aircraft_model || quote.aircraft_type || 'Aircraft';
  const client = quote.client_name || quote.client_email || 'Client';
  await createNotification({
    detailerId,
    type: 'quote_sent',
    title: 'Quote Sent',
    message: `Quote sent to ${client} for ${aircraft}`,
    link: '/quotes',
    metadata: { quote_id: quote.id },
  });
}

export async function notifySystem({ detailerId, title, message, link }) {
  await createNotification({
    detailerId,
    type: 'system',
