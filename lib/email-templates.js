...
export function airportLimitWarningTemplate({ company, plan, current, limit, deadlineIso }) {
  ...
  const content = `
    ...
    <p style="font-size:15px;color:#1a1a1a;line-height:1.6;margin:0 0 16px 0;">Your Shiny Jets CRM account currently has <strong>${current} active airport${current === 1 ? '' : 's'}</strong>, but your <strong>${planLabel} plan</strong> includes <strong>${limit} airport${limit === 1 ? '' : 's'}</strong>. We wanted to give you a heads up so nothing changes unexpectedly.</p>
    ...
    <ul style="font-size:15px;color:#1a1a1a;line-height:1.8;margin:0 0 20px 0;padding-left:20px;">
      <li>Trim your airport list down to ${limit}, or</li>
      <li>Upgrade your plan to keep all ${current} active.</li>
    </ul>
    <p style="font-size:15px;color:#1a1a1a;line-height:1.6;margin:0 0 24px 0;">If no action is taken by ${deadline}, we'll automatically deactivate your oldest excess airports so your account stays inside the ${planLabel} plan limit. Your aircraft, customers, and history all stay intact — only the extra airport rows go inactive, and they can be reactivated any time you upgrade or remove other airports.</p>
...
Your Shiny Jets CRM account has ${current} active airport${current === 1 ? '' : 's'}, but your ${planLabel} plan includes ${limit}. You have until ${deadline} to either trim your list down to ${limit} or upgrade your plan to keep all ${current} active.

If no action is taken by ${deadline}, the oldest excess airports will be deactivated automatically. Your aircraft, customers, and history stay intact — only the extra airport rows go inactive, and they can be reactivated any time you upgrade or remove other airports.
...
— The Shiny Jets team`;
  ...
  return {
    subject: `Action needed: your Shiny Jets CRM airport count exceeds your ${planLabel} plan limit`,
    html: emailWrapper(content, { headerBg: WARNING_COLOR, headerText: 'Airport limit notice' }),
    text,
  };
}

export function airportLimitEnforcedTemplate({ company, plan, limit, deactivated }) {
  const planLabel = plan ? plan.charAt(0).toUpperCase() + plan.slice(1) : 'current';
  const upgradeUrl = 'https://pricing.shinyjets.com';
  const airportsUrl = `${APP_URL}/settings/locations`;
  const list = (deactivated || []).map(d => `<li>${d.name}${d.icao ? ` (${d.icao})` : ''}</li>`).join('');
  const listText = (deactivated || []).map(d => `  - ${d.name}${d.icao ? ` (${d.icao})` : ''}`).join('\n');

  const content = `
    <p style="font-size:15px;color:#1a1a1a;line-height:1.6;margin:0 0 16px 0;">Hi ${company},</p>
    <p style="font-size:15px;color:#1a1a1a;line-height:1.6;margin:0 0 16px 0;">Your 30-day notice period ended, so we deactivated <strong>${deactivated.length} airport${deactivated.length === 1 ? '' : 's'}</strong> to bring your account inside the <strong>${planLabel} plan</strong> limit of ${limit}.</p>
...
Your 30-day notice period ended, so we deactivated ${deactivated.length} airport${deactivated.length === 1 ? '' : 's'} to bring your account inside the ${planLabel} plan limit of ${limit}.
...
— The Shiny Jets team`;
  ...
  return {
    subject: `${deactivated.length} of your airports were deactivated — Shiny Jets CRM`,
