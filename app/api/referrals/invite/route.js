...
export async function POST(request) {
  ...
  try {
    ...
    await resend.emails.send({
      from: process.env.RESEND_FROM_EMAIL || 'Vector <noreply@vectorav.ai>',
      to: email,
      subject: `${senderName} invited you to try Vector Aviation`,
      html: `<!DOCTYPE html>
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;line-height:1.6;color:#333;max-width:600px;margin:0 auto;padding:20px;background:#f0f0ee;">
  ...
  <div style="background:#fff;padding:32px 30px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px;">
    <p style="font-size:16px;margin-bottom:20px;">
      <strong>${senderName}</strong> thinks you'd love <strong>Vector Aviation</strong> — the premium quoting and client management platform for aircraft detailing professionals.
    </p>
  ...
  <p style="text-align:center;font-size:11px;color:#aaa;margin-top:16px;">Powered by <a href="https://vectorav.ai" style="color:#aaa;text-decoration:none;">Vector Aviation</a></p>
      ...
      text: `${senderName} invited you to try Vector Aviation — the premium quoting platform for aircraft detailers. Sign up here: ${signupUrl}`,
    });
  } catch (err) {
